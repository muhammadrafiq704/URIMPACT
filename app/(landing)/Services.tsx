      ...
      <div ref={container} className="flex flex-col gap-16 items-start">
        <FlexBetween className="gap-4">
          ...
          <div className=" flex flex-col gap-4">
            <p className="split font-bold text-[clamp(24px,4vw,40px)] tracking-[-0.9px] leading-11">Our Services</p>
            <p className="split font-normal text-[16px] leading-6 text-[#5B6671] ">Comprehensive environmental monitoring solutions for every need</p>
          </div>
        </FlexBetween>
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-6">
          {services.map((service) => (
            <div key={service.title} className=" relative p-8 border border-[#E2E7E980] rounded-xs flex flex-col gap-4">
              ...
              <h3 className="split font-bold text-[clamp(16px,4vw,24px)] leading-7">{service.title}</h3>

              <p className="split font-normal text-[16px] leading-6 text-[#5B6671]">{service.description}</p>
              ...
              <ul className="flex flex-col gap-2 list-none">
                {service.features.map((feature, i) => (
                  <div key={`feature-${i}`} className="flex gap-3 items-center">
                    ...
                    <li className="split font-normal text-[16px] leading-6 text-[#5B6671]">{feature}</li>
                  </div>
                ))}
              </ul>
            </div>
